...
const checkRememberMe = document.getElementById('checkRememberMe') as HTMLInputElement | null;

const userName = document.getElementById('userName')!;
const userEmail = document.getElementById('userEmail')!;
const btnSignOut = document.getElementById('btnSignOut')!;

// Cookie helper functions for 'Remember Me'
...
const checkGoogleRememberMe = document.getElementById('checkGoogleRememberMe') as HTMLInputElement | null;
...
async function handleGoogleLogin(emailParam?: string, shouldRemember?: boolean) {
  ...
  const remember = shouldRemember !== undefined
    ? shouldRemember
    : (checkRememberMe ? checkRememberMe.checked : true);
...
// About & Architecture Modal Handlers (Dynamic Single Source of Truth)
function renderMarkdownToHtml(text: string): string {
  ...
  function inlineFormat(str: string): string {
    return str
      ...
      .replace(/\*(.*?)\*/g, '<em>$1</em>')
      .replace(/`([^`]+)`/g, '<code style="background: var(--bg-primary); border: 1px solid var(--border); padding: 2px 6px; border-radius: 4px; font-family: var(--font-mono); font-size: 11px; color: var(--accent);">$1</code>');
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('# ')) {
      ...
      while (j < lines.length && !lines[j].trim().startsWith('### ') && !lines[j].trim().startsWith('# ') && !lines[j].trim().startsWith('---')) {
        ...
        if (!cLine) {
          if (cardInList) { cardBody.push('</ul>'); cardInList = false; }
          if (cardInNumList) { cardBody.push('</ol>'); cardInNumList = false; }
        } else if (cLine.startsWith('- ')) {
          if (!cardInList) { cardBody.push('<ul style="margin: 6px 0 6px 18px; padding: 0;">'); cardInList = true; }
          cardBody.push(`<li style="margin-bottom: 4px; color: var(--text-secondary);">${inlineFormat(cLine.slice(2))}</li>`);
        } else if (/^\d+\.\s/.test(cLine)) {
          if (!cardInNumList) { cardBody.push('<ol style="margin: 6px 0 6px 18px; padding: 0;">'); cardInNumList = true; }
      ...
      if (!inList) { html.push('<ul style="margin: 6px 0 6px 18px; padding: 0;">'); inList = true; }
      html.push(`<li style="margin-bottom: 4px; color: var(--text-secondary);">${inlineFormat(line.slice(2))}</li>`);
    } else if (/^\d+\.\s/.test(line)) {
      if (!inNumberedList) { html.push('<ol style="margin: 6px 0 6px 18px; padding: 0;">'); inNumberedList = true; }
...
(window as any).showAgentPromptForLink = (linkId: string) => {
...
(window as any).showAgentPrompt = (inviteId: string) => {
...
// Invite Collaborator Modal Logic
...
const inviteRecipientEmail = document.getElementById('inviteRecipientEmail') as HTMLInputElement | null;
const inviteNote = document.getElementById('inviteNote') as HTMLInputElement | null;
const inviteResultBox = document.getElementById('inviteResultBox');
const inviteUrlDisplay = document.getElementById('inviteUrlDisplay') as HTMLInputElement | null;
const btnCopyInviteLink = document.getElementById('btnCopyInviteLink');
const btnCopyInviteEmail = document.getElementById('btnCopyInviteEmail');

let lastInviteEmailTemplate: any = null;
...
let lastInviteAgentPrompt: string | null = null;
const inviteSafetyNumberDisplay = document.getElementById('inviteSafetyNumberDisplay');
const btnCopyInviteAgentPrompt = document.getElementById('btnCopyInviteAgentPrompt');
const agentPromptModal = document.getElementById('agentPromptModal');
const btnCloseAgentPromptModal = document.getElementById('btnCloseAgentPromptModal');
const btnCloseAgentPromptModalBtn = document.getElementById('btnCloseAgentPromptModalBtn');
const btnCopyModalAgentPrompt = document.getElementById('btnCopyModalAgentPrompt');
...
btnCopyModalAgentPrompt?.addEventListener('click', () => {
  ...
  if (promptText && promptText.value) {
    ...
    setTimeout(() => { if (btnCopyModalAgentPrompt) btnCopyModalAgentPrompt.textContent = '📋 Copy Prompt to Clipboard'; }, 2000);
  }
});
...
btnCopyInviteAgentPrompt?.addEventListener('click', () => {
  if (lastInviteAgentPrompt) {
    ...
    setTimeout(() => { if (btnCopyInviteAgentPrompt) btnCopyInviteAgentPrompt.textContent = '📋 Copy Prompt for Your Agent'; }, 2000);
  }
});
...
inviteForm?.addEventListener('submit', async (e) => {
  ...
  const note = inviteNote?.value.trim() || undefined;
  if (!toEmail) return;
...
btnCopyInviteLink?.addEventListener('click', () => {
  if (inviteUrlDisplay && inviteUrlDisplay.value) {
    ...
    setTimeout(() => { if (btnCopyInviteLink) btnCopyInviteLink.textContent = '📋 Copy Link'; }, 2000);
  }
});
...
btnCopyInviteEmail?.addEventListener('click', () => {
  if (lastInviteEmailTemplate) {
    ...
    setTimeout(() => { if (btnCopyInviteEmail) btnCopyInviteEmail.textContent = '📋 Copy Zero-Credential Notification Template'; }, 2000);
  }
});
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 10px; flex-wrap: wrap;">
          <div>
            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
              ...
              ${inv.safetyNumber ? `<span class="badge" style="background: rgba(16, 185, 129, 0.15); color: #10b981; border: 1px solid rgba(16, 185, 129, 0.3); font-family: var(--font-mono); font-size: 11px;">🛡️ Safety: ${escapeHtml(inv.safetyNumber)}</span>` : ''}
            </div>
            ${note}
...
function renderBugReports() {
  ...
  if (filtered.length === 0) {
    container.innerHTML = `<div style="text-align: center; padding: 16px; color: var(--text-secondary); font-size: 13px;">
      ${bugFilter === 'open' ? '🎉 No open bug reports! All systems operational.' : 'No bug reports found.'}
    </div>`;
    return;
  }
  ...
  container.innerHTML = filtered.map(b => {
    ...
    const statusBadge = b.resolved
      ? `<span class="badge badge-success" style="font-size: 10px;">✅ Resolved</span>`
      : `<span class="badge badge-danger" style="font-size: 10px; background: rgba(239, 68, 68, 0.15); color: #ef4444; border: 1px solid #ef4444;">🔴 Open</span>`;
    ...
    const resolvedMeta = b.resolved
      ? `<div style="font-size: 11px; color: #34d399; margin-top: 6px; background: rgba(16, 185, 129, 0.08); padding: 4px 8px; border-radius: 4px;">
    ...
    return `
      <div style="background: var(--bg-secondary); border: 1px solid var(--border); border-radius: 6px; padding: 12px; display: flex; flex-direction: column; gap: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 8px;">
          ...
          <div>
            ${b.resolved
              ? `<button type="button" class="btn btn-secondary btn-sm" onclick="window.toggleResolveBug('${b.id}', false)" style="font-size: 11px; padding: 4px 8px;">↩ Reopen</button>`
              : `<button type="button" class="btn btn-sm" onclick="window.toggleResolveBug('${b.id}', true)" style="font-size: 11px; padding: 4px 10px; background: #10b981; border-color: #059669; color: #fff;">✓ Mark as Resolved</button>`
            }
          </div>
        </div>
...
(window as any).toggleResolveBug = async (bugId: string, resolve: boolean) => {
...
const btnRefreshBugs = document.getElementById('btnRefreshBugs');
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Bidirectional synchronization between email inputs and checkboxes
